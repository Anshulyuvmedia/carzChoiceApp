...
const DealerDetails = () => {
    ...
    const fetchDealerCarData = async () => {
        ...
        try {
            ...
            const response = await axios.get(`https://carzchoice.com/api/myoldvehiclelist/${dealerId}`);
            // console.log('dealerCars:', response.data);
            if (response.data && response.data.oldvehicles) {
                const formattedData = response.data.oldvehicles.map((item) => {
                    ...
                    return {
                        ...
                        created_at: new Date(item.created_at).toLocaleDateString('en-GB'), // Format to dd/mm/yy
        ...
        <SafeAreaView className="flex-1 bg-gray-50">
            ...
            {loading ? (
                ...
                <FlatList
                    ...
                    data={dealerCars.slice(0, visibleItemsCount)}
                    keyExtractor={(item) => item.id.toString()}
                    renderItem={({ item }) => (
